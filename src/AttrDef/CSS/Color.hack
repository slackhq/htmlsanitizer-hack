...
class HTMLPurifier_AttrDef_CSS_Color extends HTMLPurifier\HTMLPurifier_AttrDef {
    ...
    public function validate(string $color, HTMLPurifier\HTMLPurifier_Config $config, HTMLPurifier\HTMLPurifier_Context $context) : string {
        ...
        if (\preg_match_with_matches('#(rgb|rgba|hsl|hsla)\(#', $color, inout $matches) === 1) {
            ...
            throw new \Exception("AttrDef/CSS/Color.hack pregmatch not implemented. 
            Check https://github.com/ezyang/htmlpurifier/blob/master/library/HTMLPurifier/AttrDef/CSS/Color.php for implementation.", 1);
