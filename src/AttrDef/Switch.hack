...
class HTMLPurifier_AttrDef_Switch extends HTMLPurifier\HTMLPurifier_AttrDef {
     ...
     */
    public function __construct(string $tag, HTMLPurifier\HTMLPurifier_AttrDef $with_tag, HTMLPurifier\HTMLPurifier_AttrDef $without_tag)
    {
        $this->tag = $tag;
    ...
    public function validate(string $string, HTMLPurifier\HTMLPurifier_Config $config, HTMLPurifier\HTMLPurifier_Context $context): string {
        ...
        if (!$token || (($token is Token\HTMLPurifier_Token_Text || $token is Token\HTMLPurifier_Token_Tag) 
                    && $token->name !== $this->tag)) {
