...
class HTMLPurifier_Node_Element extends HTMLPurifier\HTMLPurifier_Node {
    ...
    */
    ...
    public function toTokenPair(): (HTMLPurifier\HTMLPurifier_Token, ?HTMLPurifier\HTMLPurifier_Token) {
        if ($this->empty) {
            return tuple(new Token\HTMLPurifier_Token_Empty($this->name, $this->attr, $this->line, $this->col, $this->armor), null);
        } else {
            $start = new Token\HTMLPurifier_Token_Start($this->name, $this->attr, $this->line, $this->col, $this->armor);
            $end = new Token\HTMLPurifier_Token_End($this->name, dict[], $this->endLine, $this->endCol, $this->endArmor);
