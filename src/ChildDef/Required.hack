...
class HTMLPurifier_ChildDef_Required extends HTMLPurifier\HTMLPurifier_ChildDef {
     ...
     * @return array
     */
    public function validateChildren(vec<HTMLPurifier\HTMLPurifier_Node> $children, HTMLPurifier\HTMLPurifier_Config $_config, HTMLPurifier\HTMLPurifier_Context $_context): (bool, vec<HTMLPurifier\HTMLPurifier_Node>) {
        // Flag for subclasses
        ...
        while (C\count($stack) != 0) {
            ...
            if (($node is Node\HTMLPurifier_Node_Comment || $node is Node\HTMLPurifier_Node_Text) && $node->is_whitespace) {
                $result[] = $node;
                continue;
            }
