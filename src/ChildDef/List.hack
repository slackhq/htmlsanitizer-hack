...
class HTMLPurifier_ChildDef_List extends HTMLPurifier\HTMLPurifier_ChildDef {
    ...
    public function validateChildren(vec<HTMLPurifier\HTMLPurifier_Node> $children, 
        ...
        if ($html_def is nonnull && $html_def is Definition\HTMLPurifier_HTMLDefinition 
        ...
        foreach ($children as $node) {
            if (($node is Node\HTMLPurifier_Node_Text || $node is Node\HTMLPurifier_Node_Comment)
                && $node->is_whitespace) {
                $result[] = $node;
                continue;
            }
            ...
            if (($node is Node\HTMLPurifier_Node_Text || $node is Node\HTMLPurifier_Node_Element)
                && $node->name === 'li') {
                // good
                $current_li = $node;
                $result[] = $node;
